(function () {
    ...
    var translations = {
        ...
        'about.audioLabel': { ar: '🎧 استمع لقصتي بصوتي', en: '🎧 Listen to my story in my voice', sr: '🎧 Слушај моју причу мојим гласом', de: '🎧 Höre meine Geschichte mit meiner Stimme' },
        ...
        'banner.main':   { ar: 'الموقع الرسمي', en: 'Official Website', sr: 'Званични сајт', de: 'Offizielle Website' },
        'banner.blog':   { ar: 'المدونة', en: 'Blog', sr: 'Блог', de: 'Blog' },
    ...
    (function() {
        ...
        var trackBar = document.getElementById('audioTrackBar');
        var currentTimeEl = document.getElementById('audioCurrent');
        ...
        playBtn.addEventListener('click', function() {
            if (audio.paused) {
                ...
                playBtn.querySelector('.audio-play-icon').style.display = 'none';
                playBtn.querySelector('.audio-pause-icon').style.display = '';
            } else {
                audio.pause();
                playBtn.classList.remove('playing');
                playBtn.querySelector('.audio-play-icon').style.display = '';
                playBtn.querySelector('.audio-pause-icon').style.display = 'none';
            }
        });
        ...
        var origApply = applyLanguage;
        applyLanguage = function(lang) {
            origApply(lang);
            var langMap = { ar: 'ar', en: 'en', sr: 'sr', de: 'de' };
            var code = langMap[lang] || 'ar';
            var src = 'assets/audio/' + code + '.mp3';
    ...
    if (hero) {
        hero.addEventListener('mousemove', function (e) {
            const wrapper = this.querySelector('.hero-photo');
            if (!wrapper) return;
    ...
    if (!mosaic) { /* gallery not rendered */ }
    mosaic && mosaic.addEventListener('click', function (e) {
        if (editorEnabled) {
